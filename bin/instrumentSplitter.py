...
def separate_orchestral_stems(output_dir):
    """
    Takes the 'other.wav' generated by Demucs and uses audio-separator
    to pull out Woodwinds using the only available orchestral model.
    """
    other_stem = os.path.join(output_dir, 'other.wav')

    if not os.path.exists(other_stem):
        print("--> 'other.wav' not found, skipping orchestral extraction.")
        return

    print("\n--> Further separating 'other' into Woodwinds...")

    models = {
        "Woodwinds": "17_HP-Wind_Inst-UVR.pth"
    }

    for inst_name, model_name in models.items():
        print(f"    Extracting {inst_name} using {model_name}...")

        # Explicitly setting output format to wav to override flac defaults
        command = [
            "audio-separator", other_stem,
        ...
        try:
            ...
            print(f"    ✅ {inst_name} extraction complete.")
        except subprocess.CalledProcessError as e:
            print(f"    ❌ Error extracting {inst_name}: {e}")
        except FileNotFoundError:
            print("    ❌ 'audio-separator' not found. Ensure it is installed via pip.")
            break
...
def separate_all_stems(input_file, extract_orchestral):
    ...
    # Demucs natively outputs WAV files; no explicit flag needed for WAV.
    command = [
        "demucs",
        "-n", "htdemucs_6s",
        "--shifts", "2",
    ...
    try:
        ...
        if extract_orchestral:
            separate_orchestral_stems(output_dir)
        else:
            print("--> Skipping orchestral extraction as requested.")
...
if __name__ == "__main__":
    print("--- Minus-Lead Backing Track & Orchestral Extractor ---")
    file_name = input("Enter the filename (drag & drop file here): ").strip()
    file_name = file_name.replace("'", "").replace('"', "")

    orch_input = input("Do you want to attempt separating orchestral instruments from the 'other' stem? (y/n): ").strip().lower()
    extract_orchestral = orch_input in ['y', 'yes']

    separate_all_stems(file_name, extract_orchestral)
